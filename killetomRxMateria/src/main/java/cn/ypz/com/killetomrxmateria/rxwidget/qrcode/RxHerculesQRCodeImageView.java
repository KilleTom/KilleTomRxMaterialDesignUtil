...
public class RxHerculesQRCodeImageView extends BaseImageView {
    ...
    private boolean isTripartiteComposition;
    private RxHerculesQRCodeTools.Builder builder;
    private TripartiteComposition tripartiteComposition;
    ...
    protected void hastAttr(Context context, AttributeSet attrs) {
        ...
        isTripartiteComposition = typedArray.getBoolean(R.styleable.RxHerculesQRCodeImageView_isUsedTripartiteComposition, false);
        ...
        qrBgColor = getResources().getColor(typedArray.getResourceId((R.styleable.RxHerculesQRCodeImageView_bgColor),R.color.white));
       /*
        if (qrMode!=0) setLogo(logo);
        typedArray.recycle();*/
        RxHerculesQRCodeTools.getQrCodeTools().setApplicationContext(context);
        builder = RxHerculesQRCodeTools.getQrCodeTools().getBuilder("").
                setQrMessage(qrMessage).setSize(1200).
                setQrMessageColor(qrColor).
                setQrBGColor(qrBgColor).
                setQrLogo(logo);
    }

    public void setTripartiteComposition(TripartiteComposition tripartiteComposition) {
        this.tripartiteComposition = tripartiteComposition;
    }
    ...
    public void setQrColor(int qrColor) {
        ...
        builder.setQrMessageColor(qrColor);
        setQrMode(qrMode);
    }
    ...
    public void setLogo(int logo) {
      /*  if (size == 0) size = 1200;
        ...
        if (drawable instanceof BitmapDrawable)
            logoBitmap = ((BitmapDrawable) drawable).getBitmap().copy(Bitmap.Config.ARGB_8888, true);
        else {
            try {
    ...
    public void setQrMessage(String qrMessage) {
        ...
        builder.setQrMessage(qrMessage);
        setQrMode(qrMode);
    }
    ...
    private void logoCenter() {
        ...
        try {
            Bitmap bitmap = builder.logoCenter();
            setBitmap(bitmap);
        } catch (WriterException e) {
            Log.e("HerculesQRCodeImageView",e.getMessage());
        }
    }
    ...
    private void normal() {
         ...
         */
        try {
            Bitmap bitmap = builder.normalStyle();
            setBitmap(bitmap);
        } catch (WriterException e) {
            e.printStackTrace();
        }
    ...
    private void setBitmap(Bitmap bitmap) {
        if (isTripartiteComposition && tripartiteComposition != null)
            tripartiteComposition.tripartiteComposition(bitmap, this);
        else
            setImageBitmap(bitmap);
    }
    ...
    public interface TripartiteComposition {
        void tripartiteComposition(Bitmap bitmap, ImageView imageView);
    }
}
